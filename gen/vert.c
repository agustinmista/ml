...
int main(int argc, char **argv) {
    ...
    // Generate 0-class values centered on (-1, 0, ... , 0)
    int zeroes = n / 2;
    for (int i=0; i < zeroes; i++) { 
        fprintf(out, "%f,\t", normal_rand(-1, sigma));
        for (int j=1; j<d; j++) fprintf(out, "%f,\t", normal_rand(0, sigma));
        fprintf(out, "0\n");
    }

    // Generate 1-class values centered on (1, 0, ... , 0)
    int ones = n - zeroes;
